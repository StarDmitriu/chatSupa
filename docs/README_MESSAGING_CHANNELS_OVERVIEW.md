...
- **Параллельные рассылки:** `CAMPAIGN_SEND_SHARD_COUNT` (1 … `CAMPAIGN_SEND_MAX_SHARD_COUNT`). Если **не задано**, в коде **20** шардов — ориентир на **~20 одновременно активных** аккаунтов (кабинет + рассылки); для **~10 постоянных** пользователей с запасом тоже ок. Потолок шардов по умолчанию **64** (`CAMPAIGN_SEND_MAX_SHARD_COUNT`, максимум 256). **Один `userId`**: задачи **WA** и **TG** попадают в **один шард** и выполняются **по очереди** (в один момент не уходит параллельно в оба мессенджера). **Разные пользователи** на разных шардах могут идти параллельно. `=1` — одна очередь `campaign-send`. При N>1 воркер осушает легаси `campaign-send`; **`runSerializedByUser`** страхует гонки легаси/шард для одного пользователя.
